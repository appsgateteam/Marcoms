...
class N2NStockMoveAnalysisView(models.Model):
	...
	def init(self):
		...
		self._cr.execute("""
			...
			WHEN srcloc.usage='internal' AND desloc.usage='production' THEN 'Material Issue'
			WHEN srcloc.usage='production' AND desloc.usage='internal' THEN 'Material Issue Return'
