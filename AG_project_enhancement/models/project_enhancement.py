...
class ProjectTask(models.Model):
    ...
    material_estimation_ids = fields.One2many('material.estimate', 'material_id', 'Material Estimation', readonly=True,
                                              states={'draft': [('readonly', False)]})
    labour_estimation_ids = fields.One2many('labour.estimate', 'labour_id', 'Labour Estimation', readonly=True,
                                            states={'draft': [('readonly', False)]})
    overhead_estimation_ids = fields.One2many('overhead.estimate', 'overhead_id', 'Overhead Estimation', readonly=True,
                                              states={'draft': [('readonly', False)]})
    ...
    total_job_estimate = fields.Float(compute='_calculate_total', string='Total Job Estimate', default=0.0,
                                      readonly=True, store=True)
    state = fields.Selection([('draft', 'Draft'), ('confirmed', 'Confirmed'), ('cancel', 'Cancel')], 'Status',
                             default="draft")
    material_job_cost_line_ids = fields.One2many('materal.cost.line', 'material_job_cost_sheet_id',
                                                 'Material Job Cost Line')
    overhead_job_cost_line_ids = fields.One2many('overhead.cost.line', 'overhead_job_cost_sheet_id',
                                                 'Overhead Job Cost Line')
    overhead_line_ids = fields.One2many('account.analytic.line', 'task_id', 'Overhead Job Cost Line',
                                        domain=[('nn_overhead_id', '!=', False)])
    ...
    def actual_material(self):
        if self.name:
            ...
            res = self.env['n2n.stock.move.analysis.view'].search(
                [('analytic_id', '=', project), ('task_id', '=', task), ('source', 'in', loc),
                 ('destination', 'in', loc)])
...
class ProjectProject(models.Model):
    ...
    def _get_bill_count(self):
        for po in self:
            po_ids = self.env['account.invoice'].search(
                [('analytic_id', '=', po.analytic_account_id.id), ('type', 'in', ['in_invoice', 'in_refund'])])
    ...
    def _get_invoice_count(self):
        for so in self:
            line_id = self.env['account.invoice.line'].search(
                [('account_analytic_id', '=', so.analytic_account_id.id), ('invoice_id.type', '=', 'out_invoice')])
    ...
    def invoice_order_button(self):
        self.ensure_one()
        line_id = self.env['account.invoice.line'].search(
            [('account_analytic_id', '=', self.analytic_account_id.id), ('invoice_id.type', '=', 'out_invoice')])
        inv_lines = self.env['account.invoice.line'].browse(line_id).ids
        return {
            'name': 'Invoices',
            'type': 'ir.actions.act_window',
...
class AccountInvoice(models.Model):
    ...
    analytic_id = fields.Many2one('account.analytic.account', string="Project", readonly=True,
                                  states={'draft': [('readonly', False)]})
