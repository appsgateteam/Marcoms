...
class HelpdeskTicket(models.Model):
    ...
    def send_mail_template(self):
        ...
        for n in [1,2]:
            if n == 1 :
                ...
                template.write({'email_to': self.user_id.email})
                template.send_mail(self.ids[0], force_send=True)
            else:
                if self.partner_email:
                    template = self.env.ref('helpdesk_mgmt.assignment_email_template')
                    template.write({'email_to': self.partner_email})
                    template.send_mail(self.ids[0], force_send=True)

        # Send out the e-mail template to the user
        #self.env['mail.template'].browse(template.id).send_mail(self.ids[0], force_send=True)
        # template.send_mail(self.ids[0], force_send=True)
        # template2.send_mail(self.ids[0], force_send=True)
    ...
    def create(self, vals):
        ...
        # self.env.number = vals.get('number')
