...
class FoodExpenseSheetRegisterPaymentWizard(models.TransientModel):
    _name = 'test'
    ...
    def _default_partner_id(self):
        ...
        expense_sheet = self.env['food.expense.sheet'].browse(active_ids)
...
class transportaionExpenseSheetRegisterPaymentWizard(models.TransientModel):
    _name = 'transportaion'
    # _description = "Expense Register Payment Wizard"
    ...
    def _default_partner_id(self):
        context = dict(self._context or {})
        active_ids = context.get('active_ids', [])
        expense_sheet = self.env['transportaion.expense.sheet'].browse(active_ids)
        return expense_sheet.address_id.id or expense_sheet.employee_id.id and expense_sheet.employee_id.address_home_id.id
...
class PettyCashSheetRegisterPaymentWizard(models.TransientModel):
    _name = 'petty.cash'
    # _description = "Expense Register Payment Wizard"
    ...
    def _default_partner_id(self):
        context = dict(self._context or {})
        active_ids = context.get('active_ids', [])
        expense_sheet = self.env['petty.cash.sheet'].browse(active_ids)
    ...
    def _onchange_partner_id(self):
        active_ids = self._context.get('active_ids', [])
        expense_sheet = self.env['petty.cash.sheet'].browse(active_ids)
        # if expense_sheet.employee_id.id and expense_sheet.employee_id.sudo().bank_ids.id:
        #     self.partner_bank_account_id = expense_sheet.employee_id.sudo().bank_ids.id
    ...
    def expense_post_payment(self):
        ...
        # raise ValidationError(_(self._get_payment_vals()))
        # Create payment and post it
        payment = self.env['account.payment'].create(self._get_payment_vals())
        payment.write({'flag_acc':True})
        payment.post()
...
class LabourSheetRegisterPaymentWizard(models.TransientModel):
    _name = 'labour'
    # _description = "Expense Register Payment Wizard"
    ...
    def _default_partner_id(self):
        context = dict(self._context or {})
        active_ids = context.get('active_ids', [])
        expense_sheet = self.env['labour.sheet'].browse(active_ids)
        return expense_sheet.address_id.id or expense_sheet.employee_id.id and expense_sheet.employee_id.address_home_id.id
