...
class StockBarcodeController(http.Controller):
    ...
    @http.route('/mfg_stockbar/scan_from_main_menu', type='json', auth='user')
    ...
    def get_action(self, picking_id):
        ...
        """
        use_form_handler = request.env['ir.config_parameter'].sudo().get_param('mfg_stockbar.use_form_handler')
        if use_form_handler:
            ...
            action = request.env.ref('mfg_stockbar.stock_barcode_picking_client_action').read()[0]
            params = {
                'model': 'stock.picking',
    ...
    @http.route('/mfg_stockbar/rid_of_message_demo_barcodes', type='json', auth='user')
    def rid_of_message_demo_barcodes(self, **kw):
        ...
        action = request.env.ref('mfg_stockbar.stock_barcode_action_main_menu')
        action and action.sudo().write({'params': {'message_demo_barcodes': False}})

    @http.route('/mfg_stockbar/get_set_barcode_view_state', type='json', auth='user')
